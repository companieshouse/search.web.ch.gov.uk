...
describe("advanced search search.controller.test", () => {
    ...
    describe("check form values on results page", () => {
        ...
        it("should display the closed company status as checked when passed in the query", async () => {
            getCompanyItemStub = sandbox.stub(apiClient, "getAdvancedCompanies")
                .returns(Promise.resolve(mockUtils.getDummyAdvancedCompanyResource("test", 20)));

            const resp = await chai.request(testApp)
                .get("/advanced-search/get-results?status=closed");

            chai.expect(resp.status).to.equal(200);
            chai.expect(resp.text).to.contain("<input class='govuk-checkboxes__input' id='closedCompanies' name='status' type='checkbox' value='closed' checked>");
        });
